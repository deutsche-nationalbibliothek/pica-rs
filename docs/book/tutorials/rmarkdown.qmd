...
Beispielhaft wird in diesem Tutorial demonstriert, wie Datenanalysen mit
...
Zunächst erzeugen wir mit folgender Pica-Abfrage von der Datei `TEST.dat.gz`
...
muss das Feld `041A` mit [select]-Kommando ausgelesen werden. Sind mehrere
GND-Entitäten in einem Titel-Datensatz vohanden, expandiert das
...
$ pica select -s --no-empty-columns -H "idn_titel, idn_sw" "003@.0,041A/*.9" \
  ../TEST.dat.gz -o title_idn_gnd_idn.csv
```

Die Datei `title_idn_gnd_idn.csv` kann ggf. auch nur temporär erzeugt
werden, wenn ein Zwischenspeichern des Outputs von `pica` nicht
