...
::: {.callout-note title="Portable Version"}
Das Release `pica-X.Y.Z-x86_64-unknown-linux-musl.tar.gz` enthält eine
portable Version, die nicht von der `glibc`-Version des Systems abhängt.
:::
...
aktiviert werden. Die folgenden Funktionen können mit der `cargo`-Option
`--features` aktiviert werden:

- `performant`, um Laufzeitoptimierungen während der Kompilierung
  durchzuführen,
