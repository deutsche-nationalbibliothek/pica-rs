...
::: {.callout-warning}
Das Kommando `describe` ist eine `unstable`-Funktion, die sich noch in
der Entwicklung befindet und noch nicht in den Releases enthalten ist.
Das Kommando kann durch Aktivierung der `unstable`-Funktion beim Bauen
aus den Quellen aktiviert werden.
:::
...
beginnend und mit `R` enden, reduziert. Anschließend werden die Vorkommen
der Unterfelder ausgezähl und als Tabelle ausgegeben:
