...
Mit der Option `termination` kann die Abarbeitung der Regeln beeinflusst
werden. Der Wert `fast` legt fest, dass mit der ersten zutreffenden
Regel die Weiterverarbeitung eines Datensatzes beendet wird.
...
Weiterhin können einer Regel _Tags_ hinzugefügt werden. Anhand dieser
lassen sich bestimmte Regeln im Vorfeld mit der `--tags`-Option filtern.
...
Der `allow`-Check prüft, ob der Wert eines Unterfeldes aus einer
vorgegeben Liste stammt. Die Liste kann im CSV-, TSV- oder im
Arrow-Format vorliegen.

Im folgenden Beispiel wird überprüft, ob die DDC-Sachgruppen im Feld
`045E $e` aus der Liste `hsg.csv` stammen. Die Vergleichswerte werden
aus der Spalte `hsg` genommen.

```{.toml}
[rule.INVALID-DDC-SG]
check = 'allow'
description = 'Findet ungültige DDC-Sachgruppen im Feld 045E $e.'
link = 'https://wiki.dnb.de/display/ILTIS/5050'
list = { filename = 'hsg.csv', column = 'hsg' }
path = "045E.e"
```
#### Optionen
...
`separator = <value>`
: Nutzt den Sepator `<value>`, um mehrere Spalten zu einem Wert
zusammenzufassen. Standardmäßig wird das Zeichen `|` verwendet.
...
Der `jelc`-Check überprüft, ob ein Unterfeld einen gültigen [JEL]-Code
enthält.
...
`-r <filter>`, `--rule <filter>`
: Alle Regelwerke werden auf die Regeln reduziert, deren Bezeichnung auf
einen der spezifizierten regulären Ausdrücke passt.

`-t <tags>`, `--tags <tags>`
: Alle Regelwerke werden auf die Regeln reduziert, die einen Tag
besitzen, der aus der angegeben Liste stammt. Die Angabe der Tags
erfolgt als Liste, wobei einzelne Tags durch ein Komma getrennt werden.
Leerzeichen am Anfang oder am Ende eines Tags werden automatisch
entfernt.
