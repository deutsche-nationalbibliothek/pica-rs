...
Ob ein Unterfeld einen gültigen [ISO 639-2B]-Code enthält, lässt sich
mit dem `iso639-2b`-Check testen. Die zu überprüfenden Werte werden mit
`path` adressiert:
...
Datensatz ist im Feld `$9` hinterlegt und die Art der Beziehung
...
Datensätze gestellt werden, dass es sich auch um ein Geografikum
`Tg*` handelt. Dies lässt sich durch die Angabe eines Filer-Ausdrucks
`condition` erreichen:
...
wird aber nicht zur Bestimmung der Zieldatesätze verwendet. Besteht die
Eingabe sowohl aus Personen `Tp*` und Geografika `Tg*`, dann kann der
Geltungsbereich auf Personen eingeschränkt werden (`scope = '002@.0 =^
