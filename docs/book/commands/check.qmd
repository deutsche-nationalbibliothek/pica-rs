...
Mit der Option `termination` kann das Verahlten des Regelwerks beeinflusst werden.
Mit dem Wert `fast`, wird die Verarbeitung eines Datensatzes mit der ersten zutreffenden
Regel gestoppt.
...
Ist die Dateiendung der Ausgabedatei (Option `-o`) gleich dem Suffix
`.txt`, wird in die Ausgabedatei lediglich die PPN des Datensatzes
geschrieben (ohne Spaltenbezeichnung). Dieses Ausgabeformat eigenet sich
besonders für die Verarbeitung in der WinIBW.

Ist die Dateiendung nicht `.txt` wird standardmäßig wird eine CSV-Datei
