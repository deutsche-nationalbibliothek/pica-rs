...
Spaltenbezeichnung). Dieses Ausgabeformat eigenet sich besonders für die
...
`threshold` geändert werden. Die folgende Regel überprüft, ob es mehrere
Felder `045E` gibt, bei denen das Unter `H` mit dem Wert _dnb_ belegt
ist und die eine gleiche Belegung der Unterfelder `E` und `H` hat.

```{.toml}
[rule.DUPLICATES]
check = 'duplicates'
description = 'Findet mehrfach intellektuell vergene DDC-Sachgruppen'
link = 'https://wiki.dnb.de/display/ILTIS/5050'
query = '045E{ E, H | H == "dnb" }'
```
