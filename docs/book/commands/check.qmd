...
dienen: Die beiden Normdatensätze _Goehte_ und _Schiller_ sind im
Feld `065R` mit einem Geografikum in Beziehung gesetzt. Der verknüpfte
Datensatz ist im Feld `$9` hinterlegt und die Art der Beziehung ist im
Unterfeld `$4` kodiert. Im folgenden Beispiel sind nur die Beziehungen
_Sterbeort_ (`orts`), Geburtsort (`ortg`) und Wirkungsort (`ortw`)
relvant. In der folgenden Grafik ist das Beziehungsgeflecht dargestellt:

![](link.png){width=80% fig-align="center"}

Der einfachste Test, der sich mit dem `link`-Check formulieren lässt,
ist zu überprüfen, ob die Zieldatensätze in der Menge der Datensätze
existieren. Hierzu muss ein Pfad-Ausdruck `source` angegeben werden,
mit dem alle Identifikatoren ermittelt werden, die in den Zieldatensätzen
gesucht werden. In unserem Beispiel ist dies der Ausdruck `065R.9`, der
die PPNs der Zieldatensätze ermittelt. Als zweites wird noch ein Pfad-
Ausdruck benötigt, der die Werte im Zieldatensatz adressiert. Da der
Abgleich über die PPN eines Datensatzes erfolgt, wird der Ausdruck
`003@.0` als Ziel (`destination`) benötigt. Es ergibt sich folgende
Regel:
...
Zur Ermittlung der Ziel-Identifikatoren (`source`) können
alle Funktionen der Pfad-Ausdrücke genutzt werden. Sollen nur
`ortw`-Relationen getestet werden, kann die Regel wie folgt modifiziert
werden:
...
Darüber hinaus können noch Kriterien formuliert werden, die der
Zieldatensatz erfüllen muss. Es würde Sinn machen, an den verknüpften
Datensatz die Anforderung zu stellen, dass es sich auch um ein
Geografikum (Satzart `Tg*`) handelt. Dies lässt sich durch die Angabe
eines Filer-Ausdrucks (`condition`) erreichen:
